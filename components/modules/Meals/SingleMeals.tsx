...
export default function SingleMealsCard({
  meal,
  averageRating,
}: {
  meal: Meal;
  ...
  const handleAddToCart = async (providerId: string, mealId: string) => {
    if (!session.data) {
      return router.push("/login");
    }
    const toastId = toast.loading("Adding to cart...");
    try {
      const res = await fetch(`${backendUrl}/cart/add-to-cart`, {
        method: "POST",
        headers: {
      ...
      if (success) {
        return toast.success("Meal added to cart", { id: toastId });
      } else {
          ...
          <div className="flex flex-col bg-white lg:col-span-5">
            ...
            <CardFooter className="mt-auto flex-col gap-6 p-6 sm:p-8 lg:p-14 pt-0">
              <div className="flex w-full flex-col gap-4 sm:flex-row">
                <Button
                  onClick={() => handleAddToCart(meal.provider.id, meal.id)}
                  className="h-14 sm:h-16 flex-[1.5] rounded-2xl bg-gradient-to-r from-orange-500 to-amber-500 text-base sm:text-lg font-bold text-white shadow-xl shadow-orange-500/20 transition-all hover:scale-[1.02] hover:shadow-orange-500/30"
                >
